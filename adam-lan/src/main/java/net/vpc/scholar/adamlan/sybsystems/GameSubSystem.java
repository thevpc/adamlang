...
import net.vpc.scholar.adamlan.sybsystems.geom.*;
import net.vpc.scholar.adamlan.sybsystems.geom.Polygon;
...
//            engine.addSceneEngine(sceneEngine);
    ...
    public void draw(ExpressionEvaluator it, ExpressionNode... args) {
        prepare();
        if (args.length == 0) {
            return;
        }
        ...
        if (li.removeLocalizedVar("circle")) {
            ...
            int h = wh.y;
            Color c = AdamLanFunctionsUtils.readColor(it,li);
            draw(new net.vpc.scholar.adamlan.sybsystems.geom.Rectangle(x, y, w, h <= 0 ? w : h, c));
            ...
            Color c = AdamLanFunctionsUtils.readColor(it,li);
            draw(new net.vpc.scholar.adamlan.sybsystems.geom.Polygon(all.toArray(new Point[all.size()]), c));
